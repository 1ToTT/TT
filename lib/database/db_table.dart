...
class SqliteManager {
  ...
  Future<void> connect() async {
    try {
      ...
      String filePath = '$appDocumentsPath/$_dbname';
      print(filePath);
      File file = File(filePath);
      // Open or create the database
      // _database = await openDatabase(_dbname,
      //     version: _dbversion, onCreate: createDatabase);
       _database =  await openDatabase(file.path, version: _dbversion, onCreate: createDatabase);
    } catch (error) {
  ...
  Future<void> createDatabase(Database db, int version) async {
    try {
      await db.execute("CREATE TABLE IF NOT EXISTS TypeFinancial("
          "ID_type_financial INTEGER PRIMARY KEY AUTOINCREMENT,"
          "type_financial TEXT"
          ");");

      await db.execute("CREATE TABLE IF NOT EXISTS Financial("
  ...
  Future<void> updateData(
    ...
    try {
      await _database
          .update(tableName, data, where: 'ID_financial = ?', whereArgs: [id]);
    } catch (error) {
  ...
  Future<void> updateDataBudget(
      String tableName, Map<String, dynamic> data, int id) async {
    await connect();
    try {
      await _database
          .update(tableName, data, where: 'ID_budget = ?', whereArgs: [id]);
    } catch (error) {
      print("Error while updating data in $tableName: $error");
      rethrow;
    }
  ...
  Future<void> deleteData(String tableName, int id) async {
    ...
    try {
      await _database
          .delete(tableName, where: 'ID_financial = ?', whereArgs: [id]);
    } catch (error) {
  ...
  Future<void> deleteDataBudget(String tableName, int id) async {
    await connect();
    try {
      await _database
          .delete(tableName, where: 'ID_budget = ?', whereArgs: [id]);
    } catch (error) {
      print("Error while deleting data from $tableName: $error");
      rethrow;
    }
  ...
  Future<List<Map<String, dynamic>>> getAllBudgets() async {
    try {
      // Perform database query to retrieve all budget data
      List<Map<String, dynamic>> budgets = await _database.query('Budget');
      return budgets;
    } catch (error) {
